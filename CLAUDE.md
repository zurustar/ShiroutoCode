...
ShiroutoCode is an **agent development tool driven by a local LLM** (ローカルLLMを用いたエージェント開発ツール), per the README.

## Current state

This is a greenfield repository. As of this writing it contains only:
- `README.md` — one-line project description (Japanese)
- `LICENSE` — GNU GPL v3.0

There is **no source code, build system, dependency manifest, or tests yet**. No programming language has been chosen (`.serena/project.yml` lists `languages: []`).

When scaffolding the project, update this file with the actual build/lint/test commands and architecture once they exist — do not assume a stack until one is established here or in committed config.

## Conventions

- The project is licensed under **GPL-3.0**; new source files should be compatible with copyleft licensing.
- README content is written in Japanese; match that for user-facing docs unless directed otherwise.
