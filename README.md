...
端末で引数なしに起動すると対話モードになります。最初にモデル選択ピッカーが開き、選ぶとプロンプト入力に移ります。指示を入力するたびにエージェントが実行し、思考過程・ツール呼び出し・ステップ進捗・結果が**逐次（ストリーミングで）**表示されるので、待っている間も動作状況が分かります。プロンプト入力はUTF-8・全角対応の行編集で、日本語入力やbackspace（全角1文字をまとめて削除）も正しく動きます。`Ctrl+C` で即終了します。
...
|---|---|
| `/model` | モデルを選び直す |
| `/reset`（`/new`） | 会話履歴をクリアして新しいタスクを開始 |
| `/help` | コマンド一覧を表示 |
| `/exit`（`/quit`） | 終了（`Ctrl+C` / `Ctrl+D` でも終了） |

対話セッションは**会話を記憶**します。前の指示・生成したファイル・結果を踏まえて続けて指示できます。長い会話で文脈が大きくなりすぎたら `/reset` で新しい会話を始められます。

タスクが1回のステップ上限（既定25、`--max-steps` で変更可）で完了しなかった場合は、エージェントが**進捗を自動で要約**し（完了したこと・現状・次の具体的ステップ）、会話をその要約に圧縮して引き継ぎます。続けて「続けて」などと指示すれば、要約を踏まえて作業を継続できます（巨大化した履歴による文脈溢れも防げます）。なお、コードやファイルの作成・編集を依頼すると、エージェントは `write_file` ツールで**実際にワークスペースへ保存**します（返答テキストに表示するだけでなく）。
...
[agents.md](https://agents.md/) の慣習に従い、ワークスペース直下に `AGENTS.md` があれば、その内容をエージェントのシステムプロンプトに取り込み、プロジェクト固有の指示として従います（コーディング規約、ビルド/テストの手順、守るべき制約など）。

- 起動時に自動で読み込み、適用中は「📄 AGENTS.md のプロジェクト指示を適用中。」と表示します。
- 非常に大きい場合は安全のため先頭の一定量のみ取り込みます（コンテキスト溢れ防止）。
- 既定のエージェント方針と競合する場合は、プロジェクト固有の判断では `AGENTS.md` を優先します。
