...
対話セッションは**会話を記憶**します。前の指示・生成したファイル・結果を踏まえて続けて指示できます。長い会話で文脈が大きくなりすぎたら `/reset` で新しい会話を始められます。なお、コードやファイルの作成・編集を依頼すると、エージェントは `write_file` ツールで**実際にワークスペースへ保存**します（返答テキストに表示するだけでなく）。
