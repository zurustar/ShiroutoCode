...
```bash
# 単発実行
shiroutocode --model "google/gemma-4-12b" "READMEに使い方の節を追加して"

# 対話セッション（端末で引数なしに起動するとREPL）
shiroutocode --model "google/gemma-4-12b"
```

`--model` は必須です（または `SHIROUTO_MODEL` / 設定ファイルで指定）。LM Studio のエンドポイントが既定（`http://localhost:1234/v1`）以外なら `--endpoint` で指定します。
...
指示を1回処理し、変更内容と結果を出力して終了します。スクリプトやCIに組み込めます。

### 対話セッション（REPL）

端末で引数なしに起動すると対話モードになります。指示を入力するたびにエージェントが実行し、思考過程・ツール呼び出し・結果が逐次表示されます。
...
| モデル名（必須） | `--model` | `SHIROUTO_MODEL` | `model` | （なし） |
