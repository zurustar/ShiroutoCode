...
- [x] `application-design/components.md` — コンポーネント定義と高レベル責務・インタフェース
- [x] `application-design/component-methods.md` — 各コンポーネントのメソッド/関数シグネチャ（詳細ビジネスルールはFunctional Designで）
- [x] `application-design/services.md` — サービス定義とオーケストレーションパターン
- [x] `application-design/component-dependency.md` — 依存マトリクス・通信パターン・データフロー図
- [x] `application-design/application-design.md` — 上記を統合した設計書
- [x] 設計の完全性・整合性の検証（Security Baseline / PBT(rapid) 拡張の適用観点を含む）
...
[Answer]:実装する人がPCを行き来すると言っただけで、使う人に関しては情報はGitを通じてファイルで連携します。従ってAです
